...
function getDetectionBox(outputData) {
	...
	for (let row = 0; row < widthRange; row++) {
		...
		for (let col = 0; col < heightRange; col++) {
			...
			let channelData = outputData.slice(start * channelDepth, ( start +
				1 ) * channelDepth);
			...
			for (let box = 0; box < anchors.length / 2; box++) {
				...
				let bh = anchors[box * 2 + 1] *
					Math.exp(channelData[index + 3]);
				...
				let bestClassIndex = classPrediction.indexOf(
					Math.max(...classPrediction));
